...
- **Permission**: the calling user must have the `prvImportCustomization` privilege on each target environment that the pipeline writes to. Without it, validation succeeds for parsing but the actual deploy step fails with `SecLib::CheckPrivilege failed`. Pre-flight check: try a deploy with `autoConfirm=false`; if the validation rejects with that error, the regular account is logged in. Switch to an admin account.
...
- **Token-AppId requirement** (root cause discovered `2026-05-22`): the Pipeline-Backend's validation workflow filters by the token's `appid` claim and ignores tokens minted via Azure CLI (`appid = 04b07795-…`) or custom app registrations. Only tokens with `appid = a8f7a65c-f5ba-4859-b2d6-df772c264e9d` (Power Apps Maker, Microsoft First-Party) start validation. POSTs from non-accepted apps return HTTP 201 but the run stays in `Nicht gestartet` forever. **Solution**: the MCP server uses a separate MSAL `PublicClientApplication` configured with the Maker-AppId. Because Maker is a First-Party app and doesn't accept `http://localhost` as redirect URI, we use the **Device Code Flow** instead of interactive browser auth. One-time setup via `pipeline_auth_init` — token then cached for weeks. Read-only Pipeline calls (list/status) still use the regular token — they are not appid-gated.
